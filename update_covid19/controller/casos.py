...
def get_confirmados(df,coll):
    return df[
        ['municipio',
        'cd_geocmu',coll]
        ].groupby(['cd_geocmu','municipio']).count()


def get_by_coll(df,coll, where):
    return df[df[coll] == where][
        ['municipio',
        'cd_geocmu',
         'qtde',
        ]
...
def update():
    url_casos_confirmados = "http://datasets.saude.go.gov.br/coronavirus/casos_confirmados.csv"
    url_obitos_confirmados = 'http://datasets.saude.go.gov.br/coronavirus/obitos_confirmados.csv'
    ...
    df_casos_confirmados = casos_confirmados.rename(columns=replace).drop(
        [ 'data_inicio_sintomas', 'ano_epi', 'semana_epi',
        'raca_cor', 'diabetes', 'doenca_cardiovascular',
        'doenca_respiratoria', 'imunossupressao', 'gestante', 'puerpera',
        'regiao_saude', 'recuperado',
        'profissional_saude'],
            axis=1)
    ...
    for query in dfs:
        if query == 'queryConfimdos':
            ...
            obito = get_by_coll(
                ...
                coll,
                where).rename(columns={'qtde':query})

            confirmado = get_by_coll(
